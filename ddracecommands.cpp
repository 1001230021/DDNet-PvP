...
void CGameContext::ConInv(IConsole::IResult *pResult, void *pUserData)
{
  ...
  if(pResult->NumArguments() > 1)
  {
    pChr = GetPlayerChar(pResult->GetInteger(0));
    Ticks = pResult->GetInteger(1) * Server()->TickSpeed();
  }
  else if(pResult->NumArguments() == 1)
    pChr = GetPlayerChar(pResult->GetInteger(0));
  else
    pChr = GetPlayerChar(pResult->m_ClientID);
