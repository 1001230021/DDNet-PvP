...
void CCharacter::Snap(int SnappingClient, int OtherMode)
{
	...
	if((m_InvTimmer!=-1 && !(m_InvTimmer > 0)) || m_pPlayer->GetCID() == SnappingClient)
	...
	if(m_InvTimmer == -1 || m_InvTimmer > 0)
...
void CCharacter::InvTick()
{
	...
	if(m_SoundDelayTicks <= 0)
	{
		for(int i = 0; i < Num; ++i)
		{
			...
			if(pChr == this)
				continue;
			if(!(pChr->m_InvTimmer==-1 || pChr->m_InvTimmer > 0))
				continue;
...
const int BAR_LENGTH = 8;
	...
	else if(m_InvTimmer > 0 && m_InvTotal != 0)
	{
		float RemainingTime = (float)m_InvTimmer / m_InvTotal;

		if(RemainingTime >= 0 && RemainingTime <= 1)
		{
			char buf[BAR_LENGTH+1] = {0};
			for(int i = 0; i < BAR_LENGTH; i++)
			{
				if((float)i / BAR_LENGTH < RemainingTime)
					buf[i] = '|';
				else
					buf[i] = ' ';
			}
