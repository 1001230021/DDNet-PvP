	...
	IGameController()
{
	...
	INSTANCE_CONFIG_INT(&m_AssassinInvTime, "htn_asn_inv_time", 8, 0, 512, CFGFLAG_CHAT | CFGFLAG_INSTANCE, "刺客隐身总时长(整数, 默认8s, 限制0~268435455)");
	INSTANCE_CONFIG_INT(&m_AssassinChargeTime, "htn_asn_charge_time", 3, 0, 512, CFGFLAG_CHAT | CFGFLAG_INSTANCE, "刺客隐身倒计时(整数, 默认3s, 限制0~268435455)");
...
void CGameControllerHunter::OnCharacterTick(CCharacter *pChr)
{
  int CID = pChr->GetPlayer()->GetCID();
  int ClassID = m_Class[CID];
    
  if(ClassID == HunterClass::ID_ASSASSIN)
  {
    bool HasInput = (pChr->GetInput()->m_Direction != 0 ||
    pChr->GetInput()->m_Jump != 0 ||
    pChr->GetInput()->m_Hook != 0 ||
    (pChr->GetInput()->m_Fire & 1) != 0);

    if(pChr->GetActiveWeapon() == WEAPON_HAMMER && !HasInput)
    {
      static int s_Charge[MAX_CLIENTS] = {0};
      int CID = pChr->GetPlayer()->GetCID();

      s_Charge[CID]++;
      if(s_Charge[CID] >= m_AssassinChargeTime * Server()->TickSpeed())
      {
        pChr->SetInv(m_AssassinInvTime * Server()->TickSpeed());
        s_Charge[CID] = 0;
      }
    }
    else
...
// Assassin Class
bool CGameControllerHunter::OnAssassinFire(class CWeapon *pWeapon, vec2 Direction)
{
	if(pWeapon->IsReloading() || !pWeapon->GetAmmo())
		return true;
	...
	if(pWeapon->GetWeaponID() == WEAPON_ID_GRENADE)
	{
		...
		new CProjectile(
			...
			6.0f, // Radius
			Lifetime, // Span
			CGameControllerHunter::OnAssassinGrenadeCollide);
...
void CGameControllerHunter::OnAssassinSpawn(class CCharacter *pChr)
{
	pChr->IncreaseHealth(10);
	pChr->GiveWeapon(WEAPON_GUN, WEAPON_ID_PISTOL, 10);
	pChr->ForceSetWeapon(WEAPON_HAMMER, WEAPON_ID_HAMMER, -1);

	pChr->m_IsAssassin = true;

	GameWorld()->CreateSoundGlobal(SOUND_CTF_GRAB_EN, CmaskOne(pChr->GetPlayer()->GetCID()));
}
int CGameControllerHunter::OnAssassinDeath(class CCharacter *pVictim, class CPlayer *pKiller, int Weapon)
{
	return DEATH_NORMAL;
}
int CGameControllerHunter::OnAssassinDoDmg(class CCharacter *pChr, vec2 &Force, int &Dmg, int From, int WeaponType, int WeaponID, bool IsExplosion)
{
	if(WeaponID == WEAPON_ID_HAMMER && WeaponType == WEAPON_HAMMER)
		Dmg = 20;
	if(WeaponID == WEAPON_ID_PISTOL && WeaponType == WEAPON_GUN)
		Dmg *= 2;
	return DAMAGE_NORMAL;
}
int CGameControllerHunter::OnAssassinTakeDmg(class CCharacter *pChr, vec2 &Force, int &Dmg, int From, int WeaponType, int WeaponID, bool IsExplosion)
{
	if(pChr->GetPlayer()->GetCID() == From)
		return DAMAGE_NO_DAMAGE | DAMAGE_NO_INDICATOR;
	return DAMAGE_NORMAL;
	if(pChr->m_InvTimmer > 0)
		Dmg *= 2;
}
bool CGameControllerHunter::OnAssassinGrenadeCollide(CProjectile *pProj, vec2 Pos, CCharacter *pHit, bool EndOfLife)
{
	if(pHit && pHit->GetPlayer()->GetCID() == pProj->GetOwner())
		return false;
	...
	static const float PerAngle = pi * 2 / RAND_MAX;
	int FragNum = round_to_int(((CGameControllerHunter *)(pProj->Controller()))->m_HunterFragNumHit / 2.0f);
	vec2 FragPos = Pos;
	for(int i = 0; i < FragNum; i++) // Create Fragments
	{
		vec2 Dir = direction(PerAngle * rand());
